...
// NEON implementation of fuzzy intersection for Apple Silicon
...
// NEON implementation of sum for Apple Silicon
...
// FuzzyIntersectionSumNeon computes elementwise min between A and w using NEON SIMD
func FuzzyIntersectionSumNeon(A, w []float64, intersection_out []float64) float64 {
	size := len(A)
...
// SumFloat64Neon computes the sum of all elements in the array using NEON SIMD
func SumFloat64Neon(arr []float64) float64 {
	size := len(arr)
